...
# Напишите программу, которая определяет, может ли слон попасть с
# первой клетки на вторую одним ходом. Программа получает на вход четыре числа
...
if 1 <= x1 <= 8 and 1 <= x2 <= 8 and 1 <= y1 <= 8 and 1 <= y2 <= 8:
    ...
    summa_1 = x1 + y1
    summa_2 = x2 + y2
    if abs(delta_x) == abs(delta_y) or summa_1 == summa_2:
        print('YES')
    else:
        print('NO')
else:
